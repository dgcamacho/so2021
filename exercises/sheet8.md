...
1. Create a small program that creates arrays on the heap (using dynamic allocation). Therefore, write a function `int allocate(int n)`
   that calls itself recursively with decreasing `n` and allocates for each `n > 0` an array of integers of size `n`. The return value is `0`
   in this exercise. Do not call `delete` explicitly. In your `main()` call this function for some moderate `n`, compile your code and run it.

2. In each recursive call of `allocate()`, return the entry `n-1` of the array plus the value of the recursive call. In case of `n==0` just return `0`,
i.e. in python pseudo-code:
```python
def allocate(n):
  if n == 0:
    return 0
...
3. Instead of accessing the `n-1`th element, read from the `n`th element in the return statement of the `allocate()` function. Compile an run again!
...
that is a neutral element. Also, the associativity law should hold for all these types ad some more properties.
...
- add a combining source file `main.cc` that
  * includes all the implementation header files for the rational number type, the matrix type and the polynomial type
  ...
  * calls the `test<T>()` function with your class types.

### Hints
In order to implement the functions `identity<T>` and `random<T>`, you have to specify the for which you want to implement
...
```
The first `template <>` tells the compiler that you want to specialize this function for an explicit type, the `identity<Rational>` then
...
`std::rand() / double(RAND_MAX)` for doubles in the range `[0,1)`.
...
get a compiler error. Correct or extend your implementation to conform to the test suite. Do not modify the test suite!



## Exercise (GotW-89: Smart Pointers)
