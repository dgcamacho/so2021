...
With \cxx{17}\marginpar{[\cxx{17}]} several more operations are now sequenced and are safe to include side effects. But in general, you should not that
...
\begin{rem}
  The precedence property is not related to the order of evaluation, but to the rank of the operator. F an arbitrary function \cpp{f(a,b,c,d)} it
...
In the column \textit{Overload} means (K), the operator can only be implemented as member function of a class, whereas $\checkmark$ allows to
